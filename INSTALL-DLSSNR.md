...
This option supports DirectX 12, the DirectX 11 bridge, and native Vulkan presentation using supported SDR, HDR10 or scRGB screen formats. HDR is detected automatically; no extra HDR setting is needed. It can also change the HUD and menus. Test the look and performance in your game.
...
The INI setting is `[DlssNr] FinishedPicture=true`. Turn it off to return to your previous placement settings. DirectX 11 needs an upscaler marked w/Dx12. Vulkan uses its native NR presentation stage even when the upscaler uses the DX12 bridge. The Vulkan stage currently requires a tracked primary graphics command buffer on the presenting queue and a single swapchain in each Present call.
...
This combination is experimental. It converts the saved changes into bounded relative colour adjustments because the game's final tone mapping is not available to the mod. It can look different from running the model directly on the finished picture. It supports the SDR, HDR10 and scRGB output paths in DirectX 12 and its DirectX 11 bridge, with normal frame generation on or off. Vulkan currently supports running NR at presentation, but not this early-generation combination. Ray Reconstruction is not supported in this combination. A separate DLSS pass still has a GPU cost; a smaller NR input does not guarantee a faster overall frame.
