...
struct DlssNr_Dx12::State
{
    ...
    std::optional<bool> lastBeforeUpscale;
    ...
    std::unique_ptr<GpuTime_Dx12> ngxTime;
    ...
    bool EnsureForwarder()
    {
        ...
        nr.evaluate = (PFN_NrEvaluate) GetProcAddress(nr.forwarder, "dlssnr_call_evaluate");
        ...
        {
            nr.reason = "the forwarder is missing its exports";
            return false;
        }
    ...
    {
        if (nr.floatSlotKnown || params == nullptr || nr.probeFloat == nullptr || nr.setFloatSlot == nullptr)
            return;
    ...
    void ReleaseSurfacesIfFormatChanged(DXGI_FORMAT needed)
    {
        ...
        for (ID3D12Resource** r : { &nr.output, &nr.colorCopy, &nr.hdrCopy, &nr.colorSmall })
            ParkNrResource(*r);
    ...
    float ResolveWhitePoint(const Config& cfg, bool isHdrBuffer)
    {
        ...
        if (cfg.DlssNrWhitePointFromExposure.value_or_default() && nr.gameExposure > 1e-6f)
            return std::clamp(nr.gamePreExposure / nr.gameExposure * slider, 0.01f, 4096.0f);
                                  ...
                                  ID3D12Resource** clone)
    {
        ...
        if (*clone != nullptr)
        {
            const auto original = source->GetDesc();
            const auto previous = (*clone)->GetDesc();
            if (previous.Width != original.Width || previous.Height != original.Height ||
                previous.DepthOrArraySize != original.DepthOrArraySize || previous.MipLevels != original.MipLevels ||
                previous.SampleDesc.Count != original.SampleDesc.Count ||
                previous.Format != TypedGuideFormat(original.Format))
    ...
    {
        return nr.builtPreset == cfg.DlssNrPreset.value_or_default() &&
               nr.builtIntensity == cfg.DlssNrIntensity.value_or_default() &&
               nr.builtStyle == cfg.DlssNrStyle.value_or_default() &&
    ...
    {
        nr.builtPreset = cfg.DlssNrPreset.value_or_default();
        nr.builtIntensity = cfg.DlssNrIntensity.value_or_default();
        nr.builtStyle = cfg.DlssNrStyle.value_or_default();
        nr.builtLocalStructure = cfg.DlssNrLocalStructure.value_or_default();
        nr.builtLocalTone = cfg.DlssNrLocalTone.value_or_default();
    ...
    void ReportSkipOnce(const char* reason)
    {
        ...
        if (skipReasons.insert(reason).second)
            LOG_INFO("DLSS-NR did not run: {}", reason);
    }

    bool probeDone = false;
    std::set<std::string> skipReasons;
    ...
    bool Dispatch(DlssNr_Dx12& shader, ID3D12GraphicsCommandList* cmdList, ID3D12Resource* colour,
                  ID3D12Resource* depth, ID3D12Resource* motion, ID3D12Resource* output, const DlssNrFrameInfo& frame,
                  ID3D12CommandQueue* timingQueue)
    {
        std::lock_guard<std::mutex> nrLock(mutex);
        ...
        controls = requested;
        nr.exposureOfferedNow = frame.ExposureTexture != nullptr;
        nr.exposureEverOffered |= nr.exposureOfferedNow;
        ++nr.exposureFrames;
        struct PublishOnExit
        {
            ...
            ~PublishOnExit()
            {
                DlssNr::PublishStatus(
                    owner, DlssNr::Backend::Dx12,
                    { !state.nr.failed && (state.nr.useProxy ? state.nr.proxyRunning : state.nr.feature != nullptr),
                      state.nr.failed ? state.nr.reason : "",
                      state.lastGpuTime,
                      state.frames,
                      { state.nr.exposureFrames, state.nr.exposureOfferedNow, state.nr.exposureEverOffered,
                        state.nr.gameExposure, state.nr.gamePreExposure },
                      state.captureFrames.isActive() });
            }
        } publish { *this, &shader };
        ...
        const auto width = frame.Width != 0 ? frame.Width : static_cast<unsigned int>(colourDesc.Width);
        const auto height = frame.Height != 0 ? frame.Height : colourDesc.Height;
        if (width > desc.Width || height > desc.Height || width > colourDesc.Width || height > colourDesc.Height)
        {
            device->Release();
            ReportSkipOnce("the active colour subrect exceeds its texture");
            return false;
        }
        ...
        unsigned int guideWidth = frame.GuideWidth != 0 ? frame.GuideWidth : static_cast<unsigned int>(guideDesc.Width);
        unsigned int guideHeight = frame.GuideHeight != 0 ? frame.GuideHeight : guideDesc.Height;
        ...
        if (guideWidth > guideDesc.Width || guideHeight > guideDesc.Height || guideWidth > motionDesc.Width ||
            guideHeight > motionDesc.Height)
        {
            device->Release();
            ReportSkipOnce("the active guide subrect exceeds its texture");
            return false;
        }
        ...
        if (frame.Reset || !lastBeforeUpscale.has_value() || *lastBeforeUpscale != frame.BeforeUpscale)
            nr.reset = true;

        lastBeforeUpscale = frame.BeforeUpscale;
        ...
        if (useProxy != nr.useProxy)
        {
            ParkNrFeature(nr.feature);
            proxy.RetryAfterFailure();
            nr.useProxy = useProxy;
            nr.proxyRunning = false;
            nr.reset = true;
        }

        // The driver path must not require either the forwarder or a separate model DLL.
        // Its feature discovery uses the paths already registered with NVNGXProxy.
        const bool backendReady =
            useProxy ? ((NVNGXProxy::IsDx12Inited() || NVNGXProxy::InitDx12(device)) && proxy.Available())
                     : (EnsureForwarder() && EnsureCapabilityParams(device));
        if (!backendReady)
        {
            if (useProxy)
                nr.reason = "the NGX core does not expose the Neural Rendering proxy entry points";
            nr.failed = true;
        ...
        if (!useProxy && nr.feature != nullptr && (resolutionChanged || tuningChanged))
        {
        ...
        // Scratch textures belong to the composition pass, independently of the model backend.
        ...
        if (nr.output == nullptr || nr.colorCopy == nullptr || nr.hdrCopy == nullptr ||
            (reduced && nr.colorSmall == nullptr))
        {
            ParkNrResource(nr.output);
            ParkNrResource(nr.colorCopy);
            ParkNrResource(nr.hdrCopy);
            ParkNrResource(nr.colorSmall);
            nr.failed = true;
            nr.reason = "the composition textures could not be allocated";
            device->Release();
            return false;
        }
        ...
        if (!useProxy && nr.feature == nullptr && nr.output != nullptr && nr.colorCopy != nullptr &&
            nr.hdrCopy != nullptr)
        {
            ...
            nr.feature = nr.create(
                snippet->wstring().c_str(), ::State::Instance().NVNGX_ApplicationDataPath.c_str(), device, cmdList,
                nr.capabilityParams, workWidth, workHeight, (int) cfg.DlssNrPreset.value_or_default(),
            ...
            LOG_INFO("DLSS-NR running at {}x{}, guides {}x{} (preset {}, intensity {}, style {})", width, height,
                     guideWidth, guideHeight, nr.builtPreset, nr.builtIntensity, nr.builtStyle);
        ...
        if (!useProxy && nr.feature == nullptr)
        {
            device->Release();
            return false;
        }
        ...
        if (depthIn == nullptr || motionIn == nullptr)
        {
            ...
            LOG_ERROR("DLSS-NR unavailable: {}", nr.reason);
            if (depthIn != nullptr && depthIn == nr.depthClone)
                Barrier(cmdList, depthIn, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
                        D3D12_RESOURCE_STATE_COPY_DEST);
            if (motionIn != nullptr && motionIn == nr.motionClone)
                Barrier(cmdList, motionIn, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
                        D3D12_RESOURCE_STATE_COPY_DEST);
            device->Release();
            return false;
        }
        ...
        if (ngxTime == nullptr)
            ngxTime = std::make_unique<GpuTime_Dx12>(device);
        ...
        const bool wantExposure =
            cfg.DlssNrWhitePointFromExposure.value_or_default() && frame.ExposureTexture != nullptr;
        ...
        if (nr.meter != nullptr && wantExposure)
        {
            ...
            Barrier(cmdList, colour,
                    colour == target ? D3D12_RESOURCE_STATE_UNORDERED_ACCESS
                                     : D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
                    D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
            shader.DispatchPass(cmdList, meterParams, colour, nullptr, nullptr, (ID3D12Resource*) frame.ExposureTexture,
                                nullptr, nr.meter, nullptr);
            Barrier(cmdList, colour, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
                    colour == target ? D3D12_RESOURCE_STATE_UNORDERED_ACCESS
                                     : D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        ...
        const auto encodeParams =
            DlssNr_Common::MakeConstants(DlssNrMode_Encode, width, height, whitePoint, isHdrBuffer, cfg);

        Barrier(cmdList, colour,
                colour == target ? D3D12_RESOURCE_STATE_UNORDERED_ACCESS
                                 : D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
                D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        shader.DispatchPass(cmdList, encodeParams, colour, nullptr, nullptr, nullptr, nullptr, nr.colorCopy,
                            nr.hdrCopy);
        ...
        if (useProxy)
        {
            result = proxy.Run(cmdList, device, modelInput, depthIn, motionIn, nr.output, workWidth, workHeight,
                               guideWidth, guideHeight, nr.guideDepthInverted, nr.reset, nr.guideMvScaleX * mvToWork,
                               nr.guideMvScaleY * mvToWork, &evaluated);
            nr.proxyRunning = evaluated && result == NVSDK_NGX_Result_Success;
        }
        else
        {
            SetExtras(cfg, nullptr, nullptr, 0, 0, 0, 0);
            result = (unsigned int) nr.evaluate(
                cmdList, nr.feature, nr.capabilityParams, modelInput, depthIn, motionIn, nr.output, workWidth,
                workHeight, guideWidth, guideHeight, nr.guideDepthInverted ? 1 : 0, nr.reset ? 1 : 0,
                cfg.DlssNrIntensity.value_or_default(), (int) cfg.DlssNrStyle.value_or_default(),
                cfg.DlssNrLocalStructure.value_or_default(), cfg.DlssNrLocalTone.value_or_default(),
                cfg.DlssNrSkinStructure.value_or_default(), cfg.DlssNrAutoMask.value_or_default() ? 1 : 0,
                nr.guideMvScaleX * mvToWork, nr.guideMvScaleY * mvToWork);
        }
        ...
        if (!useProxy && !tuningReported && frames > 240)
        {
            tuningReported = true;
        ...
        if (evaluated && result == NVSDK_NGX_Result_Success)
        {
            ...
            const auto resolveParams =
                DlssNr_Common::MakeConstants(DlssNrMode_Resolve, width, height, whitePoint, isHdrBuffer, cfg);
            ...
            Barrier(cmdList, nr.output, D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
                    D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
            shader.DispatchPass(cmdList, resolveParams, modelInput, nr.output, nr.hdrCopy, motionIn, nullptr, target,
                                nullptr);
            Barrier(cmdList, nr.output, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
        ...
        if (gpuTime != nullptr)
        {
            ...
            // Timing uses the queue supplied by this feature's pipeline.
        ...
        device->Release();
        return evaluated && result == NVSDK_NGX_Result_Success;
    }
    ...
    ~State()
    {
        ...
        nr.meterFrames = 0;
        if (nr.capabilityParams != nullptr && NVNGXProxy::D3D12_DestroyParameters() != nullptr)
            NVNGXProxy::D3D12_DestroyParameters()(nr.capabilityParams);
        nr.capabilityParams = nullptr;
        if (nr.forwarder != nullptr)
            FreeLibrary(nr.forwarder);
        nr.forwarder = nullptr;
...
DlssNr_Dx12::DlssNr_Dx12(std::string InName, ID3D12Device* InDevice)
    : Shader_Dx12(InName, InDevice), _state(std::make_unique<State>())
{
    if (InDevice == nullptr)
    {
...
bool DlssNr_Dx12::Dispatch(ID3D12GraphicsCommandList* cmdList, ID3D12Resource* colour, ID3D12Resource* depth,
                           ...
                           ID3D12CommandQueue* timingQueue)
{
    return _state->Dispatch(*this, cmdList, colour, depth, motion, output, frame, timingQueue);
}
